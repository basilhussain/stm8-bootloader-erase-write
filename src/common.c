...
void flash_erase_enable(void) {
	...
	FLASH_CR2  = (uint8_t) (1 << FLASH_CR2_ERASE);
	FLASH_NCR2 = (uint8_t) ~(1 << FLASH_NCR2_NERASE);
}

void flash_block_prg_enable(void) {
	// Enable flash block writing.
	FLASH_CR2  = (uint8_t) (1 << FLASH_CR2_PRG);
...
void flash_block_prg_option_wr_enable(void) {
	...
	FLASH_CR2  = (uint8_t) ((1 << FLASH_CR2_PRG) | (1 << FLASH_CR2_OPT));
	FLASH_NCR2 = (uint8_t) ~((1 << FLASH_NCR2_NPRG) | (1 << FLASH_NCR2_NOPT));
}
